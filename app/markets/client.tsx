...
export default function MarketsPageClient({ initialData }: MarketsPageClientProps) {
  ...
  useEffect(() => {
    ...
    const fetchDataProgressively = async () => {
      // 1. Fetch critical data first (USD/GHS, Gold, GSE, Bitcoin)
      ...
      const [usdData, goldData, gseData, bitcoinData] = criticalData;
      ...
      // Mark primary sections as loaded
      setLoadingStates(prev => ({
        ...prev,
        primaryForex: false,
        primaryCommodity: false
      }));
      
      // Hide initial loading screen after critical data is shown
      if (isInitialLoad) {
        setIsInitialLoad(false);
        setHasShownContent(true);
      }
      
      // 2. Fetch other forex data
      const otherForexData = await Promise.all([
      ...
      const [eurRes, gbpRes, cnyRes, ghsNgnRes, ghsXofRes, cadRes, ghsZarRes, jpyRes, usdInterbankRes, eurInterbankRes, gbpInterbankRes] = otherForexData;
      ...
      setLoadingStates(prev => ({ ...prev, otherForex: false }));
      
      // 3. Fetch other commodities
      const otherCommodities = await Promise.all([
      ...
      const [cocoaRes, brentRes, crudeRes, coffeeRes, copperRes, steelRes, lithiumRes, rubberRes] = otherCommodities;
      ...
      setLoadingStates(prev => ({ ...prev, otherCommodity: false }));
      
      // 4. Fetch other indices
      ...
      const [nasdaqRes, ftseRes, ngxRes, cacRes, jseRes, shanghaiRes, sp500Res, daxRes] = otherIndices;
      ...
      // 6. Fetch other crypto
      const otherCrypto = await Promise.all([
      ...
      const [ethereumRes, tetherRes, xrpRes, binanceRes, chainlinkRes, cardanoRes, dogecoinRes] = otherCrypto;
      ...
      setLoadingStates(prev => ({ ...prev, crypto: false }));
      
      // 7. Fetch articles (non-critical)
      const fetchArticles = async () => {
        try {
          ...
          setCryptoArticles(cryptocurrencyRes?.data?.articles || []);
      ...
      // Fetch other indices (already started above)
      // Note: otherIndices is already awaited above
    };
    
    if (!initialData) {
      fetchDataProgressively();
    } else {
      // If initial data exists, mark all as loaded
      setIsInitialLoad(false);
      setHasShownContent(true);
      setLoadingStates({
        primaryForex: false,
  ...
  // Show loading only on initial load before any content is shown
  if (isInitialLoad && !hasShownContent) {
          ...
          <Header>
            ...
            <Currency>
              {usdData ? (
                ...
                <LoadingSkeleton style={{ height: '60px' }} />
              )}
            </Currency>
          </Header>
        ...
        <ChartItem>
          ...
          <OtherContainer>
            ...
            {Object.keys(stockStats).length > 0 ? (
              ...
              <LoadingSkeleton style={{ height: '600px' }} />
            )}
          </OtherContainer>
